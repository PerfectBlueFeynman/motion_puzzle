...
def process_data(filename, window, window_step, config, divide=True):
    ...
    """ Remove trash joints """
    global_xforms = global_xforms[:, np.array(config['chosen_joints'])]
    ...
    """ Put on Floor """
    fid_l, fid_r = np.array(config['left_foot']), np.array(config['right_foot'])
    ...
    sdr_l, sdr_r = config['shoulders']
    hip_l, hip_r = config['hips']
    across = (
    ...
    """ Foot Contacts """
    fid_l, fid_r = np.array(config['left_foot']), np.array(config['right_foot'])
    velfactor, heightfactor = np.array([0.005]), np.array([0.4])
    ...
    """ Stack all features """
    local_full = np.concatenate([local_positions, local_forwards, local_ups, local_velocities],
                                axis=-1)  # for joint-wise
    root_full = np.concatenate(
        [root_velocity[:, :, 0:1], root_velocity[:, :, 2:3], np.expand_dims(root_rvelocity, axis=-1)], axis=-1)
...
def main(config, dataset_name):
    ...
    bvh_dir = f'../database/{dataset_name}'
    ...
    test_filename = [os.path.split(x)[-1] for x in test_bvh_files]

    m_bvh_dir = f'../database/{dataset_name}_m'
    ...
    for i, item in enumerate(bvh_files):
        ...
        if os.path.split(item)[-1] in test_filename:
            local_uclip, root_uclip, ufc = process_data(item, 120, 15, config, divide=False)
            test_motions += local_uclip
            test_roots += root_uclip
            test_foot_contacts += ufc
            test_files.append(item)
        else:
            local_clips, root_clips, fcs = process_data(item, 120, 15, config, divide=True)
    ...
    for file in test_files:
        shutil.copy(file, os.path.join(test_bvh_dir, os.path.split(file)[-1]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config',
                        ...
                        help='Path to the skeleton config file.')
